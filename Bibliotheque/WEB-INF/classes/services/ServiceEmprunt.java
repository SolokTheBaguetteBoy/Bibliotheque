...
public class ServiceEmprunt extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException { 
		...
		for(Document doc : Docs){
			if(!doc.getEmprunt()){
			out.println("<br>" + doc.getType() + " " + doc.getName() + " " + doc.getId() + " <form method=\"POST\" action=\"http://localhost:8080/Bibliotheque/AbonneEmpruntDone?idDoc=" + doc.getId() + "\"><input type=\"submit\" name=\"Emprunt\" value=\"Emprunt\"></form>");
		}}
	}
}
